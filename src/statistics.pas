...
uses
  ...
  StdCtrls, CheckLst, ExtCtrls,ComCtrls, Spin, IniFiles, Dialogs, DateUtils, ToolWin, {Chart, }
  Menus, FileUtil; //, Series;
...
//    Chart3: TChart;
 ...
 //   Series3: THorizBarSeries;
    ...
    rgQC: TRadioGroup;
    GroupBox2: TGroupBox;
    chkInstrument: TCheckListBox;
    tabLog: TTabSheet;
    Memo1: TMemo;
    Memo4: TMemo;
    GroupBox3: TGroupBox;
    clbStatList: TCheckListBox;
    GroupBox4: TGroupBox;
...
begin
 ...
 PathStatistics:=GlobalPath+'unload\statistics\';
  if not DirectoryExists(PathStatistics) then CreateDir(PathStatistics);
   ...
   tabMap.TabVisible:=false;

   rgQC.ItemIndex:=0;

   Edit1.text:=floattostrF(IBLatMin,ffFixed,8,5);
   Edit2.text:=floattostrF(IBLatMax,ffFixed,8,5);
   Edit3.text:=floattostrF(IBLonMin,ffFixed,8,5);
   Edit4.text:=floattostrF(IBLonMax,ffFixed,8,5);
   
   SpinEdit1.Value:=IBDayMin;   SpinEdit2.Value:=IBDayMax;
   SpinEdit3.Value:=IBMonthMin; SpinEdit4.Value:=IBMonthMax;
   SpinEdit5.Value:=IBYearMin;  SpinEdit6.Value:=IBYearMax;

  for k:=0 to chkInstrument.Count-1 do
  if (k=0) or (k=4) or (k=7) or (k=9) then
    chkInstrument.checked[k]:=true else chkInstrument.checked[k]:=false;

   // frmdm.IBTransaction1.StartTransaction;
    odbdm.ib1q1.SQL.Text:=' select distinct StSource from Station ';
    odbdm.ib1q1.Open;
     while not odbdm.ib1q1.Eof do begin
       clbSources.Items.Add(odbdm.ib1q1.Fields[0].AsString);
      odbdm.ib1q1.Next;
     end;
   odbdm.ib1q1.Close;
...
procedure Tfrmstatistics.Button1Click(Sender: TObject);
begin
//odbdm.IBTransaction1.StartTransaction;
 with odbdm.ib1q1 do begin
   Close;
    SQL.Clear;
    SQL.Add('Select distinct(absnum) from P_SALINITY');
   Open;
 end;
 ...
 while not odbdm.ib1q1.Eof do begin
   with odbdm.ib1q2 do begin
    Close;
     SQL.Clear;
     SQL.Add('Select absnum from station');
     SQL.Add('where absnum='+inttostr(odbdm.ib1q1.Fields[0].asinteger));
    Open;
   end;
   if odbdm.ib1q2.IsEmpty then memo3.lines.add(inttostr(odbdm.ib1q1.Fields[0].asinteger));
   odbdm.ib1q1.Next;
 end;
 odbdm.ib1q1.Close;
 odbdm.IBTransaction1.Commit;
...
procedure Tfrmstatistics.clbStatListClick(Sender: TObject);
var
  I: Integer;
  Flag:boolean;
begin
Flag:=false; Count:=0;
 for I:=0 to clbStatList.Count-1 do
   if clbStatList.Checked[I] then begin
      Flag:=true;
      inc(Count);
   end;
...
procedure Tfrmstatistics.Label3Click(Sender: TObject);
Var
k:integer;
begin
 if clbStatList.Checked[0]=true then begin
  for k:=0 to clbStatList.Count-1 do clbStatList.Checked[k]:=false;
   btnStart.Enabled:=false;
 end else begin
  for k:=0 to clbStatList.Count-1 do clbStatList.Checked[k]:=true;
   btnStart.Enabled:=true;
 end;
Count:=clbStatList.Count;
end;


procedure Tfrmstatistics.lbAllSourcesClick(Sender: TObject);
Var
k:integer;
begin
 if clbSources.Checked[0]=true then begin
  for k:=0 to clbSources.Count-1 do clbSources.Checked[k]:=false;
   btnStart.Enabled:=false;
 end else begin
  for k:=0 to clbSources.Count-1 do clbSources.Checked[k]:=true;
   btnStart.Enabled:=true;
 end;
end;


procedure Tfrmstatistics.Label4Click(Sender: TObject);
Var
k:integer;
begin
 if chkInstrument.Checked[0]=true then begin
  for k:=0 to chkInstrument.Count-1 do chkInstrument.Checked[k]:=false;
   btnStart.Enabled:=false;
 end else begin
  for k:=0 to chkInstrument.Count-1 do chkInstrument.Checked[k]:=true;
   btnStart.Enabled:=true;
 end;
end;


procedure Tfrmstatistics.btnStartClick(Sender: TObject);
var
Ini:TIniFile;
...
//   showmessage(PathStatistics);
  ...
  OpenDocument(PChar(PathStatistics)); { *Converted from ShellExecute*  }
end;


procedure Tfrmstatistics.SelectAllFromStation;
Var
 k:integer;
...
begin
 with odbdm.ib1q1 do begin
    Close;
    SQL.Clear;
    SQL.Add(' Select * from Station, Station_info where ');
    ...
    SQL.Add(' Extract(Year from StDate) between :YearMin and :YearMax and ' );
    SQL.Add(' Extract(Month from StDate) between :MonthMin and :MonthMax and ' );
    SQL.Add(' Extract(Day from StDate) between :DayMin and :DayMax ' );
...
procedure Tfrmstatistics.YearStatistics;
var
k, CountSt, CountYear, YearCurrent:integer;
YearMin, YearMax:integer;
StartTime:TDateTime;
instr, source1:string;
begin
StartTime:=Now;
Assignfile(f_dat,PathStatistics+'\Year.dat'); rewrite(f_dat);
writeln(f_dat, 'Year'+#9+'Stations');
...
memo1.Lines.Add('STATION DISTRIBUTION BY YEAR:');
  ...
  YearMax:=SpinEdit6.Value;
   with odbdm.ib1q2 do begin
    Close;
      ...
      SQL.Add(' Extract(Month from StDate) ');
      SQL.Add(' between :MonthMin and :MonthMax and ' );
      SQL.Add(' Extract(Day from StDate) between :DayMin and :DayMax ' );
      if rgQC.ItemIndex=1 then SQL.Add(' and StFlag<16384 ');
 ...
 CountYear:=YearMax-YearMin+1;
  ...
  for k:=1 to CountYear do begin
   ...
   odbdm.ib1q2.Close;
   writeln(f_dat, YearCurrent, #9, CountSt, ' ');
  end;
CloseFile(f_dat);

frmmain.ProgressBar1.Position:=frmmain.ProgressBar1.Position+1;
memo1.Lines.Add(PathStatistics+'\Year.dat');
...
procedure Tfrmstatistics.MonthYearStatistics;
var
...
procedure Tfrmstatistics.MonthStatistics;
var
...
procedure Tfrmstatistics.ParameterBar;
Var
PrfCount:integer;
k_Prf:integer;
tblPar:string;
...
procedure Tfrmstatistics.VesselBar;
var
k,count_VN:integer;
...
procedure Tfrmstatistics.InstrumentBar;
var
...
procedure Tfrmstatistics.CountryBar;
var
k, Count_CN:integer;
...
procedure Tfrmstatistics.SourceBar;
var
k, count_SN:integer;
...
procedure Tfrmstatistics.Standardchartsettings1Click(Sender: TObject);
begin
 //CE.Execute;
// UpdateStationMap;
end;

procedure Tfrmstatistics.HorizontalBar;
var
...
procedure Tfrmstatistics.DayTimeDiagram;
var
...
procedure Tfrmstatistics.YearBySources;
var
ky, Count, k:integer;
YearMin, YearMax:integer;
StartTime:TDateTime;
...
procedure Tfrmstatistics.YearByCountries;
var
ky, Count, k:integer;
YearMin, YearMax:integer;
StartTime:TDateTime;
...
(*Stations on map *)
procedure Tfrmstatistics.AllStationsMap;
Var
k:integer;
lon, lat:real;
StartTime:TDateTime;
...
procedure Tfrmstatistics.AllStationsMapByYear;
Var
Ini:TIniFile;
...
procedure Tfrmstatistics.btnSaveMapClick(Sender: TObject);
begin
...
procedure Tfrmstatistics.btnCopyToClipClick(Sender: TObject);
begin
...
procedure Tfrmstatistics.TabControl1Change(Sender: TObject);
var
x,y:integer;
...
procedure Tfrmstatistics.TabControl3Change(Sender: TObject);
var
...
procedure Tfrmstatistics.TabControl2Change(Sender: TObject);
var
daycount:integer;
datecurrent:real;
st, FilePath:string;
